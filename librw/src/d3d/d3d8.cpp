...
// Decodes a D3D8 native texture into an Image and instances it as a raster of
// the CURRENT platform. The regular path below builds a PLATFORM_D3D8 raster,
// which backends without a D3D device cannot sample: on the Quest/Vulkan port
// that made every DXT and raw-format texture fall back to the 1x1 white --
// palettised textures worked (they already went through readAsImage), so peds
// and fonts were textured while roads and walls rendered white.
static Raster*
readAsImageAnyFormat(Stream *stream, int32 width, int32 height, int32 depth,
                     int32 format, int32 numLevels, int32 compression,
                     int32 hasAlpha)
{
	uint8 palette[256*4];
	int32 pallen = 0;
	...
	for(int32 i = 0; i < numLevels; i++){
		...
		if(pallen != 0){
			...
			img->setPixelsDXT(compression, data);
			if(compression == 1 && !Raster::formatHasAlpha(format))
				img->removeMask();
		}else{
			// Raw D3D layouts, little endian.
			uint8 *pixels = img->pixels;
			const uint8 *src = data;
			const int32 rasterFormat = format & 0xF00;
			for(int32 y = 0; y < img->height; y++){
				uint8 *line = pixels;
				for(int32 x = 0; x < img->width; x++){
					switch(rasterFormat){
					case Raster::C8888:	// D3DFMT_A8R8G8B8: B G R A
						line[0] = src[2]; line[1] = src[1];
						line[2] = src[0]; line[3] = src[3];
						src += 4;
						break;
					case Raster::C888:	// D3DFMT_X8R8G8B8: B G R X
						line[0] = src[2]; line[1] = src[1];
						line[2] = src[0]; line[3] = 0xFF;
						src += 4;
						break;
					case Raster::C1555: {
						uint16 c = src[0] | src[1] << 8;
						line[0] = ((c >> 10) & 0x1F) * 255 / 31;
						line[1] = ((c >>  5) & 0x1F) * 255 / 31;
						line[2] = ( c        & 0x1F) * 255 / 31;
						line[3] = c & 0x8000 ? 0xFF : 0x00;
						src += 2;
						break;
					}
					case Raster::C565: {
						uint16 c = src[0] | src[1] << 8;
						line[0] = ((c >> 11) & 0x1F) * 255 / 31;
						line[1] = ((c >>  5) & 0x3F) * 255 / 63;
						line[2] = ( c        & 0x1F) * 255 / 31;
						line[3] = 0xFF;
						src += 2;
						break;
					...
					case Raster::LUM8:
						line[0] = line[1] = line[2] = *src;
						line[3] = 0xFF;
						src += 1;
						break;
					default:
						line[0] = line[1] = line[2] = 0xFF;
						line[3] = 0xFF;
						break;
					}
					line += img->bpp;
		...
		int32 newWidth, newHeight, newDepth, newFormat;
		Raster::imageFindRasterFormat(img, Raster::TEXTURE,
			&newWidth, &newHeight, &newDepth, &newFormat);
		ras = Raster::create(newWidth, newHeight, newDepth, newFormat);
		if(ras != nil)
			ras->setFromImage(img);
	}
...
readNativeTexture(Stream *stream)
{
	...
	// A backend with no D3D device cannot sample a PLATFORM_D3D8 raster, so
	// everything -- palettised, compressed, raw -- goes through an Image and
	// comes out as a raster of the current platform.
	if(rw::platform == PLATFORM_VULKAN){
		tex->raster = readAsImageAnyFormat(stream, width, height, depth,
			format|type, numLevels, compression, hasAlpha);
		return tex;
	}
